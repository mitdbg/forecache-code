...
import abstraction.Scidb14_12IqueryTileInterface.AttributesDataPair;
import backend.disk.BigDawgTileInterface.BigDawgResponseObject;
	...
	public synchronized boolean getRawDataHelper(String query, AttributesDataPair pair, boolean retrieve_output) {
		try {
			...
			if(retrieve_output) {
				...
				getDataFromBigDawgResponseObject(bdro,pair);
			}
			return true;
		} catch(Exception e) {
			System.err.println("error occured while sending request to big dawg");
