...
public class NewTileKey implements java.io.Serializable {
	...
	public double getDistance(NewTileKey other) {
		int[] oid = other.dimIndices;
		// ignore invalid input
		if((this.dimIndices.length != oid.length) || (this.zoom < 0) || (other.zoom < 0)) {
			return -1;
		}
		int sum = 0;
		sum += Math.abs(this.zoom - other.zoom);
		for(int i = 0; i < this.dimIndices.length; i++) {
			sum += Math.abs(this.dimIndices[i] - other.dimIndices[i]);
		}
		return sum;
	}
