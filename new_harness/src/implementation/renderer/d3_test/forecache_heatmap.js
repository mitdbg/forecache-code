...
ForeCacheHeatmap.getPrefix = function() { return BigDawgVis.getPrefix() + "forecache-heatmap-";};
...
ForeCacheHeatmap.getVis = function(root,o,jsondata) {
  var name = ForeCacheHeatmap.getPrefix()+BigDawgVis.uuid();
  var visDiv = $("<div id=\""+name+"\"></div>").appendTo(root);
  var options = ForeCacheHeatmap.updateOpts(o,jsondata.table);
  var graph = new HeatmapObj(visDiv,options,jsondata.table);
  return visDiv;
};

// adds to the default options set in BigDawgVis.updateOpts
ForeCacheHeatmap.updateOpts = function(opts,table) {
  ...
  xstats = ForeCacheHeatmap.get_stats(table,opts,newopts.xname);
  console.log(xstats);
  ...
  ystats = ForeCacheHeatmap.get_stats(table,opts,newopts.yname);
...
ForeCacheHeatmap.get_stats = function(table,opts,sort) {
  var stats = {};
  if(table.length == 0) {
    return stats;
  }
