...
public class Client {
		...
		for(String taskname : tasknames) {
			for(int i = 0; i < models.length; i++) {
				for(int predict = 0; predict < predictions.length; predict++) {
					crossValidation(taskname,models[i],predictions[predict],false);
				}
			}
		}
	}
	...
	public static void crossValidationModelSpecific(int[] users, String[] tasknames, String[][] models, int[] predictions, boolean[] usePhases) throws Exception {
		...
		for(String taskname : tasknames) {
			for(int predict = 0; predict < predictions.length; predict++) {
				for(int i = 0; i < models.length; i++) {
					crossValidation(taskname,models[i],testusers,new int[]{predictions[predict]},usePhases[i]);
				}
			}
		}
	}
	...
	// test all users
	public static void crossValidation(String taskname, String[] models, int predictions, boolean usePhases) throws Exception {
		List<Integer> testusers = DBInterface.getUsers();
		crossValidation(taskname,models,testusers,new int[]{predictions}, usePhases);
	...
	public static void crossValidation(String taskname, String[] models, List<Integer> testusers, int[] predictions,
		...
		for(int u1 = 0; u1 < testusers.size(); u1++) {
			...
			sendReset(trainlist,models,predictions,taskname,usePhases);
	...
	public static void getTracesForSpecificUsers(int[] user_ids, String[] tasks, String[] models, int predictions, boolean usePhases) throws Exception {
		...
		for(int u = 0; u < user_ids.length; u++) {
			...
			for(int t = 0; t < tasks.length; t++) {
				...
				System.out.println("found trace of size " + trace.size() + " for task '" + taskname + "' and user '" + user_id + "'");
				long average = 0;
				sendReset(user_ids,models,new int[]{predictions},taskname,usePhases);
				for(int r = 0; r < trace.size(); r++) {
					UserRequest ur = trace.get(r);
					String tile_id = ur.tile_id;
					String tile_hash = ur.tile_hash;
	...
	public static boolean sendReset(int[] user_ids, String[] models, int[] predictions,
			String taskname, boolean usePhases) {
		String urlstring = "http://"+backend_host+":"+backend_port+"/"+backend_root + "/"
				+ "?"+buildResetParams(user_ids,models, predictions, taskname,usePhases);
		URL geturl = null;
		HttpURLConnection connection = null;
		BufferedReader reader = null;
		StringBuffer sbuffer = new StringBuffer();
	...
	public static String buildResetParams(int[] user_ids, String[] models, int[] predictions, String taskname, boolean usePhases) {
		String params = "reset&taskname=";
		params += taskname;
