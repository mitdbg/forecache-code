...
import implementation.modis.ModisAllocationStrategyMapFactory;
import implementation.modis.TestModisViewFactory;
...
import abstraction.mdstorage.MainMemoryTileBuffer;
import abstraction.mdstorage.TileBuffer;
import abstraction.mdstorage.TileCacheManager;
import abstraction.mdstorage.eviction.LruPolicy;
import abstraction.mdstorage.MultiDimTilePacker;
...
import abstraction.mdstructures.SessionMetadata;
import abstraction.mdquery.MultiDimTileInterface;
import abstraction.mdquery.Scidb13_3IqueryTileInterface;
import abstraction.mdquery.Scidb14_12IqueryTileInterface;
import abstraction.mdquery.Scidb14_12OptimizedIqueryTileInterface;
import abstraction.structures.View;
import abstraction.tile.MultiDimColumnBasedNiceTile;
import abstraction.tile.MultiDimTileDecoder;
import abstraction.util.DBInterface;
import abstraction.util.UtilityFunctions;

public class ModisMainThread {
	public static MultiDimTileRetrievalHelper trh;
	...
	public static void main(String[] args) throws Exception {
		...
		//MultiDimTileStructure ts = OldModisTileStructureFactory.getDefaultModisTileStructure();
	...
	public static class FetchTileServlet extends HttpServlet {
		...
		protected void makePredictions() {
			//TODO: FIX PREDICTION CODE!!!!! Doesn't use multi dim structures
			//List<MultiDimTileKey> predictions = predictionEngine.getPredictions(md, dtv, neighborhood);
			List<MultiDimTileKey> predictions = new ArrayList<MultiDimTileKey>();
			cacheManager.insertPredictions(predictions);
		}
		
		protected void updateAccuracy(MultiDimTileKey id) {
		...
		protected void doRemoveTile(HttpServletRequest request,
			...
			MultiDimTileKey key = new MultiDimTileKey(
					UtilityFunctions.parseTileIdIntegerNoBrackets(tile_id,"_"),
					UtilityFunctions.parseZoom(zoom));
			boolean returnval = ((Scidb14_12OptimizedIqueryTileInterface) dtv.nti)
		...
		protected void doJsonFetch(HttpServletRequest request,
			...
			MultiDimColumnBasedNiceTile t = null;
			try {
		...
		protected void doBinaryFetch(HttpServletRequest request,
			...
			try {
				...
				// send the response
				long s = System.currentTimeMillis();
				byte[] toSend = MultiDimTileDecoder.getBytes(t);
				long e = System.currentTimeMillis();
		...
		protected void doFetch(HttpServletRequest request,
			...
			try {
				...
				// send the response
				long s = System.currentTimeMillis();
				byte[] toSend = MultiDimTilePacker.serializeTile(t);
				long e = System.currentTimeMillis();
				response.getOutputStream().write(toSend,0,toSend.length);
				long e2 = System.currentTimeMillis();
				//String report= (s-ns)+","+(e-s)+","+(e2-e)+","+toSend.length;
		...
		protected void doAddHistory(HttpServletRequest request,
			...
			int[] z = UtilityFunctions.parseZoom(zoom);
			MultiDimTileKey key = new MultiDimTileKey(id,z);
			...
			MultiDimColumnBasedNiceTile t = cacheManager.getTileForMetadata(key);
		...
		protected MultiDimColumnBasedNiceTile fetchTile(String tile_id, String zoom) throws InterruptedException {
			String reverse = UtilityFunctions.unurlify(tile_id); // undo urlify
			int[] id = UtilityFunctions.parseTileIdInteger(reverse);
			int[] z = UtilityFunctions.parseZoom(zoom);
			MultiDimTileKey key = new MultiDimTileKey(id,z);
