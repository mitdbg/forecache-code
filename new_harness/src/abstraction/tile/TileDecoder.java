...
public class TileDecoder {
	...
	public static ColumnBasedNiceTile readBytes(byte[] data) {
		int offset = 0;
		ColumnBasedNiceTile tile = new ColumnBasedNiceTile();
		tile.id = new NewTileKey(new int[]{},0);
	...
	public ColumnBasedNiceTile getTile(byte[] encodedTile) {
		ColumnBasedNiceTile tile = new ColumnBasedNiceTile();
		return tile;
	}
	...
	/***************** Helper Functions ********************/
	public static byte[] packKey(ColumnBasedNiceTile tile) {
		byte[] data = new byte[(tile.id.dimIndices.length+2)*doubleSize];
		ByteBuffer buffer = ByteBuffer.wrap(data);
		int offset = 0;
		buffer.putDouble(offset,tile.id.zoom);
		offset += doubleSize;
	...
	public static int unpackKey(byte[] data, int offset, NewTileKey id) {
		ByteBuffer buffer = ByteBuffer.wrap(data);
		id.zoom = (int) buffer.getDouble(offset);
		offset += doubleSize;
	...
	public static byte[] packAttributes(ColumnBasedNiceTile tile) {
	...
	public static byte[] packDataTypes(ColumnBasedNiceTile tile) {
	...
	public static void main(String[] args) {
		...
		byte[] encodedTile = TileDecoder.getBytes(t);
		ColumnBasedNiceTile t2 = TileDecoder.readBytes(encodedTile);
